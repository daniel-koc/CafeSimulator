...
ProductDescription(char m, const std::string& n, double p): marker(m), name(n), price(p) {}
ProductDescription(const ProductDescription& d): marker(d.marker), name(d.name), price(d.price) {}
...
* This is class for drinks and add-ons and they differ by the type of the marker.
  */
class ProductsMenu {
 protected:
...
ProductsMenu(std::string menu_name, IsCorrectProductMarker is_correct_product_marker): menu_name_(menu_name), is_correct_product_marker_(is_correct_product_marker) {}
 ...
 * @param marker Used to lookup in the menu.
  ...
  *
  * @return True when a product was found for the given marker; false otherwise.
*/
...
/**
  ...
  *
  * @return True when product descriptions could be read from the given file; false otherwise.
  */
