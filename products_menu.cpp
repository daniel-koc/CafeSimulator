    ...
    if (is_correct_product_marker_(data[i])) {
      ...
      if (description_len > 0) {
        ...
        do {
          i++;
          description_len--;
        } while (description_len > 0 && !(data[i] >= 'a' && data[i] <= 'z') &&
                 !(data[i] >= 'A' && data[i] <= 'Z'));

        if (description_len > 0) {
          // If a name was found:
          int name_len = 1;
          while (name_len < description_len && data[i + name_len] != '(' &&
                 data[i + name_len] != '$')
            name_len++;
          ...
          if (name_len < description_len) {
            ...
            if (price_len > 0) {
              ...
              while (data[k + price_len - 1] < '0' &&
                     data[k + price_len - 1] > '9')
                price_len--;
              ...
              try {
                ...
                std::cerr << "Exception: No conversion to double could be "
                          ...
                          << std::string(&data[k], price_len) << "'!"
                          << std::endl;
              } catch (std::out_of_range e) {
                std::cerr << "Exception: The string value '"
                          << std::string(&data[k], price_len)
                          << "' is out of the range of representable values by "
                             "an double!" << std::endl;
              }
            }
          }
